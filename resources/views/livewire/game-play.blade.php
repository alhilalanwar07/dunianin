...
>
    <header class="flex flex-col items-center justify-between gap-3 bg-orange-50 px-4 py-3 shadow-sm sm:flex-row sm:px-6 sm:py-4">
        <div class="text-center sm:text-left">
            <h1 class="text-2xl font-extrabold text-amber-900 sm:text-3xl">Dunia Anin</h1>
            <p class="text-base font-bold text-orange-700 sm:text-xl">Halo, {{ $player?->username ?? 'Pemain' }}</p>
        </div>
        <div class="flex w-full flex-col items-center gap-2 text-center sm:w-auto sm:items-end sm:text-right">
            ...
            <a href="{{ route('leaderboard') }}" wire:navigate class="inline-block rounded-full bg-orange-200 px-4 py-1.5 text-xs font-bold text-orange-700 shadow-sm transition hover:bg-orange-300 sm:bg-transparent sm:px-0 sm:py-0 sm:text-lg sm:text-orange-600 sm:underline sm:shadow-none sm:hover:bg-transparent sm:hover:text-orange-800">
                Leaderboard
            </a>
        </div>
    </header>
        ...
        <section wire:key="view-arena" wire:transition.scale.duration.300ms class="flex-1 p-5">
            @if ($currentChallenge)
                ...
                >
                    ...
                    <div class="mb-4 flex flex-col items-center justify-between gap-3 text-center sm:flex-row sm:gap-0 sm:text-left">
                        <p class="text-2xl font-extrabold text-amber-900 sm:text-3xl" x-text="challenge.prompt"></p>
                    ...
                    <template x-if="challenge.engine === 'macro_dnd'">
                        <div class="flex flex-1 flex-col gap-4 p-2 sm:flex-row sm:gap-6">
                            <div class="flex w-full flex-wrap items-center justify-center gap-3 rounded-3xl bg-white/50 p-4 shadow-inner sm:w-3/5 sm:gap-6 sm:rounded-[2rem] sm:p-6" style="min-height: 180px;">
                                @for ($i = 0; $i < ($currentChallenge['payload']['spawn_count'] ?? 3); $i++)
                                    <div
                                        class="touch-none touch-target flex h-16 w-16 cursor-grab items-center justify-center rounded-2xl bg-gradient-to-br from-violet-400 to-fuchsia-400 shadow-xl transition-all duration-300 hover:scale-110 hover:-rotate-6 active:cursor-grabbing active:scale-95 sm:h-28 sm:w-28 sm:rounded-3xl"
                                        draggable="true"
                                        @dragstart="drag({{ $i }})"
                                        x-show="!items[{{ $i }}]?.done"
                                    ...
                                    >
                                        <div class="pointer-events-none absolute inset-0 rounded-2xl border-2 border-white/30 sm:border-4 sm:rounded-3xl"></div>
                                        <x-svg-icon name="{{ $currentChallenge['payload']['target_asset'] ?? 'apel' }}" class="h-10 w-10 text-white drop-shadow-lg sm:h-20 sm:w-20" />
                                    </div>
                                @endfor
                            </div>

                            <div
                                class="flex w-full items-center justify-center rounded-3xl border-4 border-dashed border-amber-400 bg-amber-100/50 p-4 shadow-lg transition-all duration-300 sm:w-2/5 sm:rounded-[2rem] sm:border-8 sm:p-6"
                            ...
                            >
                                <div class="text-center transition-transform duration-300" :class="draggedIndex !== null ? 'scale-110' : ''">
                                    <div class="mx-auto mb-2 flex h-20 w-20 items-center justify-center rounded-full bg-white/50 shadow-inner sm:mb-4 sm:h-40 sm:w-40">
                                        <x-svg-icon name="keranjang" class="h-12 w-12 text-orange-500 drop-shadow-md sm:h-28 sm:w-28" />
                                    </div>
                                    <p class="text-lg font-extrabold text-amber-900 drop-shadow-sm sm:text-3xl" x-text="draggedIndex !== null ? 'Lepaskan!' : 'Taruh di sini'"></p>
                                </div>
                            </div>
                        </div>
                    </template>
