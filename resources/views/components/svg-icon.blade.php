@php
    $iconName = trim($name);
    $viewName = 'components.svg.' . $iconName;
    $availableAssets = array_merge(config('svg-assets.assets', []), config('svg-assets.ui', []));
@endphp

@if (in_array($iconName, $availableAssets, true) && view()->exists($viewName))
    @include($viewName)
@else
