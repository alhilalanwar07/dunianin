@php
    ...
    $uiIcons = config('svg-assets.ui', []);
    $isUiIcon = in_array($iconName, $uiIcons, true);
...
@if ($isUiIcon && in_array($iconName, $availableAssets, true) && view()->exists($svgrepoViewName))
    @include($svgrepoViewName)
@elseif (in_array($iconName, $availableAssets, true) && view()->exists($legacyViewName))
    @include($legacyViewName)
@elseif (in_array($iconName, $availableAssets, true) && view()->exists($svgrepoViewName))
    @include($svgrepoViewName)
@else
